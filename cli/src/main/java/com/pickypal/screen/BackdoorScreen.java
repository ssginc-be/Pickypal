...
public class BackdoorScreen {
    ...
    public static void start(LoginResponseDto loginInfo) throws IOException {
        ...
        while(true) {
            Console.clear();
            System.out.println("[ [가상] 배송 처리 화면 ]");
...
//            System.out.println("---------------------------------------------------------");
    ...
    public static void shipItem(Long orderId) {
        // API 호출
        ApiKit apiKit = new ApiKit();
        String endpoint = "http://localhost:8080/backdoor/ship?order_id=" + orderId;

        ApiResponse response = apiKit.getRequest(endpoint);
    }
